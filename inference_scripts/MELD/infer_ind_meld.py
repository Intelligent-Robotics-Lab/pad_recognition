"""
Inference script for single-modality PAD prediction on MELD. Matches train_ind_meld.py
exactly (on-the-fly feature extraction) and mirrors
inference_scripts/IEMOCAP/infer_ind.py's structure, adapted for MELD's single fixed test
...
# Calculation for all of the metrics to be reported
...
checkpoint = f"saved_models/best_meld_{MODALITY}_model.pth"

if not os.path.exists(checkpoint):
    raise FileNotFoundError(f"No checkpoint found at {checkpoint} — run train_ind_meld.py first.")
